...
1. Sleep Score 가중치 변경 시 What's New 또는 변경 안내가 필요한가?
   - **결정**: 예. 0.7.0 What's New에 Sleep Score 5축 재설계를 안내한다.
   - **근거**: 점수 계산 방식 변경은 사용자가 인지해야 할 핵심 변경.
...
3. 야간 바이탈 대시보드에서 SpO2를 별도 트랙으로 표시할지, HR과 동일 축에 배치할지?
   ...
   - **근거**: SpO2(%)와 HR(bpm)은 단위/범위가 달라 동일 축 배치 시 가독성 저하.

4. 수면-운동 상관분석을 Sleep 탭에 배치할지, Activity 탭에 배치할지?
   - **결정**: Sleep 상세 (MetricDetailView, sleep 조건부 섹션).
   - **근거**: 수면 관점에서 운동 영향을 보는 것이 사용자 플로우에 자연스러움. Activity에서 수면으로의 역방향 링크는 Future로 보류.
...
- [x] `/plan 2026-03-28-sleep-analysis-enhancement` 으로 구현 계획 생성 (2026-03-28)
- [x] Phase별 TODO 생성 (2026-03-29)
- [x] MVP Phase 1~4 구현 완료 (PR #642, #650, #655, #661, #669)
- [x] Open Questions 결정 반영 (2026-03-29)
- [ ] Nice-to-have 항목 구현 (Future)
