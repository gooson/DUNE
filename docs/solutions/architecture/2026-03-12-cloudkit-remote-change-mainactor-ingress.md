...
`DUNEApp` handler는 `mainThreadPublisher(for:)`로 publisher delivery를 main run loop에 고정한 뒤
`Task { @MainActor in ... }` 내부에서만 coordinator를 읽도록 바꿨다.
...
| File | Change | Reason |
|------|--------|--------|
| `DUNE/Data/Services/NotificationCenter+MainThread.swift` | main-thread notification publisher helper 추가 | background-delivered NotificationCenter ingress를 선언적으로 main으로 고정 |
| `DUNE/App/DUNEApp.swift` | remote change handler를 main-thread publisher + main actor helper 경유로 변경 | `@State appRuntime` 접근을 background task 밖으로 격리 |
...
- [ ] NotificationCenter 기반 app lifecycle ingress가 `@State`, `@Observable`, `ObservableObject`를 읽거나 쓰면 `receive(on: RunLoop.main)` 또는 `mainThreadPublisher(for:)`로 delivery thread를 먼저 고정한다.
