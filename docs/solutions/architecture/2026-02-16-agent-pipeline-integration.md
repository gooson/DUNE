---
tags: [agent, pipeline, workflow, quality-gate, code-review, lfg, work-skill]
category: architecture
date: 2026-02-16
severity: important
related_files: [.claude/skills/work/SKILL.md, .claude/skills/lfg/SKILL.md, .claude/agents/]
related_solutions: []
---
...
- `/work`, `/lfg` 실행 시 6관점 코드 리뷰만 수행되고 UI/성능/품질 전문 검증 누락
- PR 생성 전 최종 검증 단계 부재

### Root Cause

에이전트가 도구로 등록되어 있으나, `/work`와 `/lfg` 스킬의 파이프라인 정의에 포함되지 않아 수동 호출에만 의존.
...
| `.claude/skills/lfg/SKILL.md` | Phase 3.5 Quality Agents 추가 | 리뷰와 해결 사이 품질 에이전트 단계 |
| `.claude/skills/lfg/SKILL.md` | Phase 6에 pr-reviewer 추가 | PR 생성 전 최종 검증 |
...
- [ ] 새 에이전트 추가 시 `/work` 또는 `/lfg` 파이프라인에 통합 여부 검토
...
3. **파이프라인 단계 번호 설계**: `/lfg`에서 Phase 3.5를 추가한 것처럼, 기존 번호 체계를 깨지 않으면서 단계를 삽입할 수 있는 유연성 필요
