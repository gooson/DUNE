...
| `DUNEUITests/Smoke/LaunchExperienceSmokeTests.swift` | add | UI test용 강제 automatic `What's New` 진입 smoke coverage 추가 |
...
- **Files**: `DUNEUITests/Smoke/LaunchExperienceSmokeTests.swift`, `DUNEUITests/Manual/HealthKitPermissionUITests.swift`
- **Changes**: UI test 전용 강제 automatic sheet hook 추가, smoke/manual 경로 모두 known row 존재를 확인한 뒤 닫도록 테스트 보강
- **Verification**: automatic `What's New` launch path가 blank sheet를 pass시키지 않는지 확인
...
- Integration tests: `LaunchExperienceSmokeTests`, `HealthKitPermissionUITests`에서 automatic `What's New` row 존재 확인
...
| UI test용 강제 진입 hook이 production flow에 섞일 위험 | low | medium | `--force-automatic-whatsnew` 인자에서만 실행되도록 제한 |
