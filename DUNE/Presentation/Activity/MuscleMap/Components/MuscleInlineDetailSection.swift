...
/// Inline section showing recovery stats and top exercises for a selected muscle.
/// Replaces MuscleDetailPopover when shown within MuscleMapDetailView.
struct MuscleInlineDetailSection: View {
            ...
            if let state = fatigueState {
                ...
                if state.compoundScore != nil {
                    ...
                    .sheet(isPresented: $showingInfoSheet) {
                        if let score = fatigueState?.compoundScore {
    ...
    private func statsGrid(state: MuscleFatigueState) -> some View {
        ...
        return LazyVGrid(columns: columns, spacing: DS.Spacing.sm) {
            statItem(title: "Fatigue Level", value: "\(state.fatigueLevel.shortLabel) / L10", icon: "flame.fill")
            statItem(title: "Weekly Volume", value: "\(state.weeklyVolume.formattedWithSeparator) sets", icon: "chart.bar.fill")
            statItem(title: "Last Trained", value: lastTrainedText(state: state), icon: "clock.fill")
            if let score = state.compoundScore {
                statItem(
                    title: "Sleep Modifier",
                    value: "\(score.breakdown.sleepModifier.formattedWithSeparator(fractionDigits: 2))\u{00d7}",
                    icon: "moon.fill"
                )
            } else {
                statItem(title: "Base Recovery", value: "\(Int(muscle.recoveryHours).formattedWithSeparator)h", icon: "timer")
            }
        }
    }
