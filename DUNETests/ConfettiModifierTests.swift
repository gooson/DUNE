...
struct ConfettiModifierTests {
    ...
    func burstZeroCount() {
        let particles = ConfettiParticle.burst(count: 0)
        #expect(particles.isEmpty)
    }

    @Test("all shape types represented in burst")
    func allShapesRepresented() {
        let particles = ConfettiParticle.burst(count: 9)
        let shapes = Set(particles.map(\.particleShape))
    ...
    func particleGravityEffect() {
        var particle = ConfettiParticle.burst(count: 1)[0]
    ...
    func particleNotExpiredAtStart() {
        let particle = ConfettiParticle.burst(count: 1)[0]
        #expect(!particle.isExpired)
    }
