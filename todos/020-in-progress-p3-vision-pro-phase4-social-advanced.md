...
- 이번 배치는 `/run`으로 G1 foundation(SharePlay workout board), E1, F3 foundation(speech capture + parser + draft preview)까지 ship 완료.
- SharePlay의 SharedWorldAnchors/spatial alignment, voice persistence, body tracking, full-avatar는 capability 및 실기기 검증이 추가로 필요하므로 후속 구현으로 남김.
