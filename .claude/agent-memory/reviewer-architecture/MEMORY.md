...
### SymmetryDetail struct co-location in Domain/Services
- `SymmetryDetail` struct is defined at the bottom of `PostureAnalysisService.swift` (Domain/Services), not in Domain/Models
- Rule: Domain data structs used across Presentation belong in `Domain/Models/` — burying them inside a service file hides discoverability and couples the model to the service file
- Pattern: move `SymmetryDetail` to `Domain/Models/PostureMetric.swift` (alongside `PostureMetricResult`, `PostureStatus`) or its own `Domain/Models/SymmetryDetail.swift`

### PostureReportGenerator localization gap
- `PostureReportGenerator` (Presentation/Posture) uses bare NSString literals for all PDF section headings ("Posture Assessment Report", "Overall Score", "Assessment Details", "Notes", "Generated by DUNE", "Metric", "Value", "Status", "Confidence")
- UIGraphicsPDFRenderer output is locale-sensitive but these strings bypass `String(localized:)` entirely
- Fix: wrap each label with `String(localized:)` before casting to NSString

### PostureReminderScheduler reuses BedtimeReminderNotificationScheduling protocol (correct)
- `PostureReminderScheduler` correctly reuses the existing `BedtimeReminderNotificationScheduling` protocol and `UserNotificationCenterBedtimeReminderScheduler` implementation from `BedtimeWatchReminderScheduler.swift`
- Pattern is consistent with `BedtimeReminderScheduler` — both inject via protocol, both are `@MainActor final class` with `.shared` singleton in Data/Services
- No pattern inconsistency found; protocol naming (`BedtimeReminderNotificationScheduling`) is mismatched to its new consumer but is a P3 rename concern

### Calendar computation in View body
- `PostureHistoryView.reminderBanner` computes `Calendar.current.dateComponents([.day], ...)` inside a `@ViewBuilder` computed property — runs on every render cycle
