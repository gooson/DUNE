...
2. **Uncommitted 변경 확인**: `git status`로 커밋되지 않은 변경 확인
   - 변경사항이 있으면 자동 처리:
     - 1차: `git add -A && git commit -m "chore: finalize before ship"`
     - 2차(커밋 실패 시): `git stash push -u -m "ship-preflight-{timestamp}"`
   - 커밋과 stash가 모두 실패하면 즉시 중단하고 실패 원인 보고
3. **리모트 동기화**: 현재 브랜치가 리모트에 push되었는지 확인
   - 안 되어 있으면 `git push -u origin {branch}` 실행
4. **리뷰 finding 재조정**: 이전 `/review` 결과가 있으면 현재 `main...HEAD` diff 기준으로 다시 맞춥니다
