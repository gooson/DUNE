...
- `.onPreferenceChange` → `@State` → `.background { .environment() }` 패턴 금지 → `.backgroundPreferenceValue` 사용. `@State` 중간 단계가 body re-evaluation을 트리거하여 `@Observable` 추적과 합쳐지면 navigation 전환 시 feedback loop 발생
